...
class umrah extends CI_Controller {
    ...
    public function index() {		
		...
		$data["master_title"] = "Umrah Lists";   // Please enter the title of page......
		$data["master_body"] = "index";   // Please enter the body of page......
		$data['userdata'] = $this->session->userdata('userdata');
		$this->load->theme('umrah', $data);  // Loading theme		
    }
	...
	public function add_new() {	
		...
		if($reqData){			
			...
			$this->db->insert('gp_umrah',$reqData);
		...
		$data["master_title"] = "Add New Umrah";   // Please enter the title of page......
	...
	public function edit($id=null){
		...
		if($this->input->post()){
			$reqData = $this->input->post();
			if(!isset($reqData['publish'])){
				$reqData['publish'] = 0;
			}
			$this->db->where('id',$id);
			$this->db->update('gp_umrah',$reqData);
			redirect(BASEURL.'/umrah');
		}
		
		$data['results'] = $query->row_array();
	
		$data["master_title"] = "Edit  Umrah";   // Please enter the title of page......
