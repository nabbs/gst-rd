...
$("#open_gallery").on('click',function(){
	...
	$("#open-gallery-box").on("hidden.bs.modal", function () {
		$.ajax({
		type: "POST",
		url: "/admin/sliders/get_save_image",
		dataType: 'json',
		success: function(result){  
		 if(result.image_name){
		$(".col-sm-4").html('<img class="img-responsive image_selected" src="/../assets/files/'+result.image_name+'" alt="Photo" data-src="/../assets/files/'+result.image_name+'"><input type="hidden" id="package_logo" name="pic" value="'+result.image_name+'">');
		 }
		}
		});
	});
