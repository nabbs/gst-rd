		...
		<div class="row">
			<div class="col-12">
				<div class="sectionTitle">
					<h2 class="text-uppercase u-heading-v8__title  g-font-size-27 mb-25"><span>Hajj</span></h2>
...
<div class="row g-py-12">	
	...
	<!--<div class="row">
		<div class="col-md-3 col-xs-3 col-sm-3 col-lg-3">    
			<h3 class="gp_white_color hjj_filter"><span class="refine">REFINE</span><span class="refine_options"> OPTIONS</span></h4>
		</div>
		<div class="col-md-9 col-xs-9 col-sm-9 col-lg-9">  
			<h3 class="gp_white_color packahes-found hjj_filter">1 Results</h3>  
		</div>
	</div>-->

<div class="col-sm-3 hajj_search_filters">
<h3 class="gp_white_color hjj_filter"><span class="refine">REFINE</span><span class="refine_options"> OPTIONS</span></h3>
	<div class="panel panel-default ">
		<div class="panel-body hajj_filter_body">
					...
					<div class="panel-body range_price">
						...
						<input class="simple-slider" type="hidden" value="0,6000"/>           
					</div>
	...
	<div class="col-sm-8" >
	 <h3 class="gp_white_color packahes-found hjj_filter">Hajj Results</h3>  

		<h3 class="gp_white_color hajj_resultss"><span class="no-of-packages"><?= count($get_hajj); ?></span>&nbsp;Packages Found</h3>	

		<?php if(empty($get_hajj)){ ?>
		<div class="panel panel-default">
			<div class="panel-body">      
				<div class="col-sm-12" style="text-align: center;margin-bottom: 20px;"><b>Hajj Package Not Available</b>
				</div>  
			</div>  
		</div>  
		<?php } ?>

		<div id="loading" style="left: 0;top: 0;width: 100%;">
			<?php $image = base_url(). 'assets/images/Gear.gif'; ?>
			<img style="position: absolute;display: inline;left: 32%;margin-top: 15%; display:none;" class='loader_img' src="<?php echo $image; ?>" />
		</div>
		<div id="results"></div>	
	<?php 
	if(!empty($get_hajj)){  
	...
	height="270" frameborder="0" src="https://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q=' . str_replace(",", "", str_replace(" ", "+", $val['hotel_address'])) . '&z=14&output=embed"></iframe>';
	$iframe1 = '<iframe width="100%" 
	height="270" frameborder="0" src="https://maps.google.com/maps?f=q&source=s_q&hl=en&geocode=&q=' . str_replace(",", "", str_replace(" ", "+", $val['medina_hotel_address'])) . '&z=14&output=embed"></iframe>';
	...
	if(!empty($get_hajj)){ 
	$rowCount = 1;
	foreach($get_hajj as $val):   
	if ($rowCount++ % 2 == 1 ){ $class="hajj_results_even"; }
	else{$class="hajj_results_odd";}  
	$hj_imgg = '/../assets/files/100X100/'.$val['package_logo']; 
	if($rowCount <= 6){

	?>
	<li class="jplist-panel box panel-top" style="list-style:none;">
	...
	<div class="col-md-2 col-sm-2 col-xs-2 col-lg-2 hajj_first_section" style=" background: url(<?= $hj_imgg ?>) no-repeat; height: 130px;  background-size:contain;">
	...
	<?php for($i=1;$i<=$val['room_rating'];$i++){ ?>
	<i class="fa fa-star yellow_star" aria-hidden="true"></i> 
	<?php }
	...
	<li><b>Medina Hotel:</b></li>
	<li class="hajj-royal-scot-medina"><span><?= $val['medina_hotel']; ?></span> 
	<?php for($i=1;$i<=$val['medina_room_rating'];$i++){ ?>
	<i class="fa fa-star yellow_star" aria-hidden="true"></i>
	<?php			}
	...
	<li class="hajj-more-infoo gp_view_more">More Info</li>
	</ul></div>
	<div class="col-md-4 col-sm-4 col-xs-4 col-lg-4 hajj_third_section">
           <ul>
		   <?php if(@$val['makkha_room_type_quad'] || @$val['makkha_room_type_triple'] || @$val['makkha_room_type_double'] ){ ?>
		   <li><span class="room_type_heading"><?php echo @$val['makkha_room_type_quad']; ?></span>
		   <span class="room_type_heading"><?php  echo @$val['makkha_room_type_triple']; ?></span>
		    <span class="room_type_heading"><?php  echo @$val['makkha_room_type_double']; ?></span>
		   </li>
		   <?php } 
		   if(@$val['makkha_room_type_quad_price'] || @$val['makkha_room_type_triple_price'] || @$val['makkha_room_type_double_price'] ){
		   ?>
		   
        <li><?php if(@$val['makkha_room_type_quad'] and @$val['makkha_room_type_quad_price']){ ?><span class="hajj_room_type"><?php echo @$val['makkha_room_type_quad_price']; ?></span><?php } ?>
		
		<?php if(@$val['makkha_room_type_triple'] and @$val['makkha_room_type_triple_price']){ ?><span class="hajj_room_type"><?php echo @$val['makkha_room_type_triple_price']; ?></span><?php } ?>
		
		<?php if(@$val['makkha_room_type_double'] and @$val['makkha_room_type_double_price']){ ?><span class="hajj_room_type"><?php echo @$val['makkha_room_type_double_price']; ?></span><?php } ?>
	
		</li>
		<?php } ?>
		
		<li><b>Room Basis:</b> <?php echo @$val['room_basis']; ?></li>
          <li><b>Shifting:</b><?php if(@$val['shifting']){ echo 'Yes'; }else{ echo 'No';}?></li>
            <li><b>Flights Included:</b><?php if(@$val['flights_included']){ echo 'Yes'; }else{ echo 'No';}?></li>
      </ul>
        </div>
		<a href="<?php echo @$val['website_url']; ?>" target="_blank" class="btn gp_view_more-old hajj_view_detail pull-right">VIEW DEAL</a>
	<div class="gp_more_details" style="display:none;">
	<ul>
	<li class="row">
	...
	<div class="row gp_more_details_map" style="display:none;"><div class="inner_map_details">
	<ul class="abc">
	<li style="list-style: none;">
	<div class="col-sm-8 col-md-8 col-xs-8 col-lg-8 "><b>Makkah Hotel Address   </b></div>
	<div class="col-sm-4 col-md-4 col-xs-4 col-lg-4 pull-right makkah_close"><span>&#10006;</span></div>
	</li>
	<li style="list-style: none;">
	...
	<div class="row gp_more_details_map1" style="display:none;"><div class="inner_map_details">
	<ul class="abc">
	<li style="list-style: none;">
	<div class="col-sm-8 col-md-8 col-xs-8 col-lg-8 "><b>Medina Hotel Address   </b></div>
	<div class="col-sm-4 col-md-4 col-xs-4 col-lg-4 pull-right medina_close"><span>&#10006;</span></div>
	</li>
	<li style="list-style: none;">
	...
	?>
	<?php if($get_hajj){ ?>
	<div class="custom_pagination pull-right index_page ">
	<?php if ( ($pagenum-1) > 0) {
	?>  
