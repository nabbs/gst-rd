    ...
    if ($this->Sender != '' && strlen(ini_get('safe_mode'))< 1) {
      ...
      if ($this->SingleTo === true && count($toArr) > 1) {
        foreach ($toArr as $key => $val) {
          $rt = @mail($val,mb_encode_mimeheader($this->Subject,$this->CharSet, 'B', ''), $body, $header, $params);
          // implement call back function if it exists
          $isSent = ($rt == 1) ? 1 : 0;
          $this->doCallback($isSent,$val,$this->cc,$this->bcc,$this->Subject,$body);
        }
      } else {
        $rt = @mail($to,mb_encode_mimeheader($this->Subject,$this->CharSet, 'B', ''), $body, $header, $params);
        // implement call back function if it exists
        $isSent = ($rt == 1) ? 1 : 0;
      ...
      if ($this->SingleTo === true && count($toArr) > 1) {
        ...
        $rt = @mail($to,mb_encode_mimeheader($this->Subject,$this->CharSet, 'B', ''), $body, $header);
        // implement call back function if it exists
        $isSent = ($rt == 1) ? 1 : 0;
        $this->doCallback($isSent,$to,$this->cc,$this->bcc,$this->Subject,$body);
      }
